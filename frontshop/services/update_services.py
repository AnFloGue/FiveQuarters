...
def update_basket_item(basket_item_id, quantity, basket_id, product_id):
    url = f"{API_BASE_URL}/basketitems/{basket_item_id}/update/"
    headers = get_headers()
    data = {
        'quantity': quantity,
        'basket': basket_id,
    ...
    try:
        ...
        print(f"Error updating basket item: {e} - {response.text if response else 'No response'}")
